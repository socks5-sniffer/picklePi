  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="space-y-2">
        ...
        <p className="text-slate-500">A record of your experiments, failures, and learnings.</p>
      </header>
      ...
      <div className="space-y-6">
        {entries.map(entry => {
          ...
          return (
            <div key={entry.id} className="bg-white rounded-2xl p-8 shadow-sm border border-slate-200">
              <div className="flex justify-between items-start mb-6 border-b border-slate-100 pb-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-900">{project?.title || 'Unknown Project'}</h2>
                  <div className="text-sm text-slate-500 flex items-center gap-2 mt-1">
                    <span className="bg-slate-100 px-2 py-0.5 rounded text-xs font-semibold uppercase tracking-wider">Level {project?.level}</span>
                    <span className="flex items-center gap-1"><Calendar size={14} /> {date}</span>
                  </div>
                </div>
                {project && (
                  <div className="bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full text-sm font-bold border border-emerald-200 shadow-sm flex items-center gap-2">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <p className="text-slate-600 text-sm leading-relaxed bg-slate-50 p-4 rounded-xl border border-slate-100 min-h-[5rem]">
                    {entry.whatWorked}
                  </p>
                </div>
                <div className="space-y-2">
